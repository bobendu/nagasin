...
// Initialisation de Stripe (Cl├® publique factice pour la d├®mo)
...
export default function StoreApp({ isAdmin, adminToken, onLogout, onGoToLogin }: { isAdmin?: boolean, adminToken?: string, onLogout?: () => void, onGoToLogin?: () => void }) {
  ...
  const handleAddProduct = () => {
    const newProduct: Product = {
      ...
      category: 'EDITION',
      price: 0,
      description: 'Description ├á compl├®ter...',
      details: '',
  ...
  const handlePublish = async () => {
    try {
      ...
      alert("Changements publi├®s avec succ├¿s ! Ils sont maintenant synchronis├®s sur tous les appareils.")
    } catch (err) {
  ...
  const handleOrderSuccess = async (paymentId: string) => {
    ...
    try {
      ...
      console.log("Commande sauvegard├®e sur le serveur.");
    } catch (err) {
    ...
    <div className="app-layout">
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav" style={{ padding: '2rem 1.5rem' }}>
          {/* L'acc├¿s au panier a ├®t├® d├®plac├® dans le bandeau du bas */}
        </nav>
      ...
      <main className="main-content">
        <header style={{ marginBottom: '4rem' }}>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <p style={{ fontSize: '1.1rem', color: 'var(--text-muted)', maxWidth: '600px', lineHeight: 1.5 }}>
              ...
              Ouvrages, dessins et curiosit├®s graphiques.
            </p>
          </motion.div>
        </header>
        ...
        <section style={{ borderTop: '1px solid #eee', marginTop: '6rem', padding: '4rem 0 10rem 0', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '1rem', maxWidth: '500px', margin: '0 auto 2rem' }}>
            Pour toute question sur votre commande ou un projet sp├®cifique, n'h├®sitez pas ├á me contacter.
          </p>
              ...
              <div style={{ flex: 1, overflowY: 'auto', paddingRight: '0.5rem' }}>
                {cart.length === 0 && cartStep !== 'success' ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
                          ...
                          <div key={index} style={{ borderBottom: '1px solid #f9f9f9', paddingBottom: '1.5rem' }}>
                            <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start', marginBottom: '1rem' }}>
                              ...
                              <div style={{ flex: 1 }}>
                                ...
                                <span style={{ fontSize: '0.8rem', fontWeight: 800 }}>{item.price} Ôé¼</span>
                              </div>
                              <Trash2 
                            ...
                            {item.canBeDedicated && (
                              <div style={{ background: '#fcfcfc', padding: '1rem', border: '1px dashed #eee' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '0.5rem' }}>
                                  ...
                                  <span style={{ fontSize: '0.7rem', fontWeight: 800, color: 'var(--primary-color)' }}>D├ëDICACE PERSONNALIS├ëE</span>
                                </div>
                                <textarea 
                                  placeholder="├Ç qui dois-je d├®dicacer ce tirage ?"
                    ...
                    {cartStep === 'checkout' && (
                      /* ├ëTAPE 2 : FORMULAIRE CLIENT D├ëTAILL├ë */
                    ...
                    {cartStep === 'success' && (
                      ...
                      >
                        ...
                        <h3 style={{ fontSize: '1.5rem', fontWeight: 900, marginBottom: '1rem' }}>COMMANDE R├ëUSSIE !</h3>
                        <p style={{ color: '#666', fontSize: '0.9rem', lineHeight: 1.6, marginBottom: '2rem' }}>
                          Merci pour votre achat. Un email de confirmation a ├®t├® envoy├® ├á <strong>{customerInfo.email}</strong>.
                        </p>
                        <button 
                ...
                <div style={{ borderTop: '1px solid #eee', paddingTop: '2rem', marginTop: '2rem' }}>
                  <div style={{ padding: '1.5rem', borderTop: '1px solid #eee', background: '#fafafa', borderRadius: '12px', marginTop: '2rem', marginBottom: '2rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem', marginBottom: '0.5rem' }}>
                      ...
                      <span style={{ fontWeight: 600 }}>{subtotal.toFixed(2)} Ôé¼</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem', marginBottom: '1rem' }}>
                      ...
                      <span style={{ fontWeight: 600 }}>{shippingCost.toFixed(2)} Ôé¼</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1.2rem', fontWeight: 900 }}>
                      <span>TOTAL</span>
                      <span>{total.toFixed(2)} Ôé¼</span>
                    </div>
                  </div>
      ...
      >
        ...
        {justAdded ? 'ARTICLE AJOUT├ë !' : `VOIR MON PANIER (${cart.length})`}
      </motion.div>
    </div>
  )
}
